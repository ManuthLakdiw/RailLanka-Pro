...
function loadStations(page) {
    fetch(`http://localhost:8080/api/v1/raillankapro/station/getAll/${page}/7`,requestOptions)
            .then(response => response.json())
            .then(data => {
            ...
            if (stations.length === 0) {
                $("#stationTable tbody").append('<tr><td colspan="6" class="px-6 py-4 text-center text-gray-500">No data available</td></tr>');
            } else {
                ...
                <tr class="hover:bg-gray-50">
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center">
                      <div
                        class="flex-shrink-0 h-10 w-10 bg-${randomColor}-100 rounded-full flex items-center justify-center"
                      >
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm text-gray-900 font-mono">${station.stationCode}</div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <span
                      class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full status-active ${statusClass}"
                    >
                      ${statusText}
                    </span>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <div class="flex space-x-2">
                      <button id="updateStationBtn" 
                              type="button" 
                              class="text-blue-600 hover:text-blue-900"
                              data-id="${station.stationId}">
                              <i class="fas fa-edit"></i>
                      </button>
                      <button id="deleteStationBtn" 
                              type="button"  
            ...
            .catch(err => {
            console.log(err);
            $("#stationTable tbody").html('<tr><td colspan="6" class="px-6 py-4 text-center text-red-500">Error loading data</td></tr>');
            });
}
...
$("#filterStation").on("input" , function () {
    const keyword = $(this).val().trim(); 
      ...
      if (keyword.length >= 3) { 
        const myHeaders = new Headers();
        myHeaders.append("Authorization", "Bearer eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJtYW51MjAwNiIsImlhdCI6MTc1NTUyODMwNywiZXhwIjoxMDc1NTUyODMwN30.YfaJiYMfcueirAjqy00EmxXfF9w1r9edNyq9UbSyXeo");
        myHeaders.append("Content-Type", "application/json");
        ...
        fetch(`http://localhost:8080/api/v1/raillankapro/station/filter?keyword=${keyword}`, requestOptions)
        .then(response => response.json())
        .then(result => {
            console.log(result);
            ...
            if (stations.length === 0) {
              $("#stationTable tbody").append(`<tr>
                                          ...
                                          No stations found matching your search keyword: <span class="font-bold text-gray-900">${keyword}</span>
                                          </td>
              </tr>`);
                ...
                stations.forEach(station => {
                    ...
                    $("#stationTable tbody").append(`
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${station.noOfPlatforms} platform(s)</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full status-active ${statusClass}">
                                    ${statusText}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                <div class="flex space-x-2">
                                    <button id="updateStationBtn" type="button" class="text-blue-600 hover:text-blue-900" data-id="${station.stationId}">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button id="deleteStationBtn" type="button" class="text-red-600 hover:text-red-900" data-id="${station.stationId}">
                                        <i class="fas fa-trash"></i>
                                    </button>
                                    <button class="text-gray-600 hover:text-gray-900">
                                        <i class="fas ${station.inService ? "fa-toggle-on" : "fa-toggle-off"}"></i>
